...
### E. 電腦版表格拉滿規範 (Table Width for Desktop)
*   **預設左右拉滿**：在電腦版（非手機/大螢幕）下，畫面上展示的資料表格 (Table) 預設必須左右拉滿整個畫面（容器寬度佔滿 100%），不可以將資料侷限或擠在窄小的中間區塊（如限制 1200px），以發揮大螢幕的橫向空間優勢。只有在手機版 (Mobile) 等窄小螢幕下，才可透過響應式 (RWD) 限制容器最大寬度。
